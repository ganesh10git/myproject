 ...
 */
(function($) {
  Drupal.behaviors.im_user_getdynamics_form = {};
  Drupal.behaviors.im_user_getdynamics_form.attach = function(context) {
    $tabs = $(".table_global");
    $( ".table-trans-replace tbody" ).sortable({
      ...
      stop: function(){ 
        ...
        var search_data = $('#edit-getdynamics-textbox').val(); 
        var data = "data=" + $('#edit-textbox').val();
        $.ajax({
          url: '/admin/im/getdynamics/"'+search_data+'"/ajax',
          type: 'POST',
          data: data,
          success: function(a) {
          }
