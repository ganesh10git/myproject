...
Drupal.behaviors.autosave.attach = function (context, settings) {
  ...
  if ($('#autosave-status').size() == 0 && isClick==0) {
    ...
    setup: function (e, o) {
      ...
      if (autosaveSettings.savedTimestamp) {
        showingRestoreCommand = true;
         // Save and prepare for regional moderation is only displaying for regional content
         $('#edit-save-prepare').hide();
          ignoreCallbackPath = Drupal.settings.autosave.basePath + '/autosave/ignore/' + autosaveSettings.formid + '/' + autosaveSettings.savedTimestamp + '/' + autosaveSettings.formToken + '/' + autosaveSettings.theme;
          ignoreLink = $('<a>').attr('href', ignoreCallbackPath).addClass('use-ajax').attr('title', Drupal.t('Ignore/Delete saved form')).html(Drupal.t('Ignore')).click(function (e) {
          ...
          callbackPath = Drupal.settings.autosave.basePath + '/autosave/restore/' + autosaveSettings.formid + '/' + autosaveSettings.savedTimestamp + '/' + autosaveSettings.formToken + '/' + autosaveSettings.theme;
